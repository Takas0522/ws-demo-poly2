...
class TenantService:
    ...
    async def create_tenant(
        self, request: TenantCreate, created_by: str = "system"
    ) -> TenantResponse:
        """Create a new tenant"""
    ...
    async def get_tenant(self, tenant_id: str) -> TenantResponse:
        ...
        if not tenant:
            raise HTTPException(
                status_code=404, detail={"code": ErrorCode.NOT_FOUND, "message": "テナントが見つかりません"}
            )

        return self._map_to_response(tenant)

    async def update_tenant(
        self, tenant_id: str, request: TenantUpdate, updated_by: str = "system"
    ) -> TenantResponse:
        """Update tenant"""
        # Get existing tenant
    ...
    async def delete_tenant(self, tenant_id: str, deleted_by: str = "system") -> bool:
    ...
    async def assign_tenant_admin(
        self, tenant_id: str, request: AssignAdminRequest, assigned_by: str = "system"
    ) -> dict:
        ...
        existing = await self.tenant_user_repo.get_by_user_and_tenant(request.user_id, tenant_id)
        if existing:
            raise HTTPException(
                status_code=409,
                detail={"code": ErrorCode.CONFLICT, "message": "ユーザーは既にこのテナントに関連付けられています"},
            )
    ...
    def _map_to_response(self, tenant_data: dict) -> TenantResponse:
        ...
        return TenantResponse(
            ...
            deleted_at=datetime.fromisoformat(tenant_data["deletedAt"])
            if tenant_data.get("deletedAt")
            else None,
        )
