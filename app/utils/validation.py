...
def validate_email_domain(email: str, tenant: dict) -> bool:
    ...
    if not allowed_domains:
        raise HTTPException(status_code=400, detail="テナントに許可ドメインが設定されていません")
    ...
    allowed_lower = [d.lower().lstrip("@") for d in allowed_domains]

    # Check if domain is in allowed list
    if domain_lower not in allowed_lower:
        raise HTTPException(
            status_code=400, detail=f"許可されたドメイン（{', '.join(allowed_domains)}）のメールアドレスを使用してください"
        )
...
def validate_domain_list(domains: List[str]) -> bool:
    ...
    for domain in domains:
        ...
        if not clean_domain or " " in clean_domain:
            raise HTTPException(status_code=400, detail=f"無効なドメイン形式: {domain}")

        # Check for at least one dot
        if "." not in clean_domain:
            raise HTTPException(
                status_code=400, detail=f"無効なドメイン形式: {domain}（ドメインには少なくとも1つのドットが必要です）"
            )
